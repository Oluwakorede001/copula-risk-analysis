...
```text
├── notebooks/
│   ├── Problem_1.ipynb              # Structural reliability analysis (Jupyter)
│   └── Problem_2.ipynb              # Portfolio risk analysis (Jupyter)
├── data/
│   ├── Data_I.csv                   # Building I — 10,000 nonlinear structural analyses
│   ├── Data_II.csv                  # Building II — 10,000 nonlinear structural analyses
│   └── Data_Prob_2.csv              # Portfolio asset returns (N = 800 paired observations)
├── figures/                         # All generated plots (auto-populated on run)
├── report/
│   └── HW5_Report.pdf               # Full technical report
└── README.md
```
...
```math
\mu_{\ln} = \frac{1}{N}\sum_{n=1}^{N}\ln(x_n)
```

```math
\sigma_{\ln} = \sqrt{\frac{1}{N-1}\sum_{n=1}^{N}\left(\ln(x_n)-\mu_{\ln}\right)^2}
```
...
$$
v_1 = F_{S_a}(s_a), \qquad v_2 = F_{\mathrm{EDP}}(\mathrm{edp})
$$
...
**Gaussian Copula** — parameterized by $\rho_0$, estimated as the Pearson correlation of the probit-transformed pseudo-observations:

$$
\hat{\rho}_0 = \mathrm{corr}\left(\Phi^{-1}(F_{S_a}(s_a)), \Phi^{-1}(F_{\mathrm{EDP}}(\mathrm{edp}))\right)
$$

**Clayton Copula** — parameterized by $\theta > 0$, estimated using Kendall's rank correlation:

$$
\hat{\theta} = \frac{2\hat{\tau}}{1-\hat{\tau}}
$$
...
$$
\log L = \sum_{n=1}^{N} \log c\left(F_{S_a}(s_{a,n}), F_{\mathrm{EDP}}(\mathrm{edp}_n)\right)
$$
...
#### 3. Monte Carlo Simulation (N_sim = 500,000)
...
- **Capital buffer**: `F_Y⁻¹(10⁻³)` — the 0.1th percentile of the portfolio loss distribution
...
|---|---|---|
| `μ̂_ln` (Sa) | 5.392 | 5.389 |
| `σ̂_ln` (Sa) | 0.468 | 0.470 |
| `μ̂_ln` (EDP) | 1.294 | 1.293 |
| `σ̂_ln` (EDP) | 0.427 | 0.429 |
...
Open `notebooks/Problem_1.ipynb` for the structural analysis and `notebooks/Problem_2.ipynb` for the portfolio risk analysis. All figures are saved automatically to the `figures/` directory.
...
```
The Clayton copula has **positive lower-tail dependence** with coefficient `λ_L = 2^{−1/θ}`. As `θ → ∞`, dependence becomes perfect; as `θ → 0`, the variables become independent.

### Conditional CDF Derivation

For the Gaussian copula:
```
F(edp | sa) = Φ( [Φ⁻¹(F_EDP(edp)) − ρ̂₀ Φ⁻¹(F_Sa(sa))] / √(1−ρ̂₀²) )
```
...
| Portfolio PDF | `f_Y(y)` under both copula models with VaR markers |
